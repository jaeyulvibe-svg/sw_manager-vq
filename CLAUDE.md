...
- **Migrated** (query real tables, no mock data standing in for query results): `dashboard-view.tsx`, `assets-view.tsx`, `patch-view.tsx`, `kisa-view.tsx`, `notifications-view.tsx`, `notice-boards.tsx`, `sw-master-view.tsx` (`sw_masters` table, editable draft grid via `sw-master/use-master-draft.ts`), `servers-view.tsx` (`servers` table, "admin-servers" nav entry, Source URL 관리 스타일의 즉시 반영형 CRUD), `request-view.tsx` (`asset_requests` + `notifications`; product picker sources from `sw_masters`, 설치 서버 드롭다운은 `servers`에서), `approval-view.tsx` (`asset_requests` read/update, inserts into `assets` on approval, `notifications`), and the presentational dashboard widgets they feed (`charts.tsx`, `kpi-cards.tsx`, `critical-alerts.tsx`, `asset-charts.tsx`).
