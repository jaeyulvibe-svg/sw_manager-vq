# KISA/제조사 취약점 공지 분리 및 승인 현황 개편 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split the single "KISA 취약점 공지" screen into three screens (KISA-only review, 제조사-only review, and a notice-centric "승인된 취약점 공지" board), backed by a new `source_type` column on `vulnerabilities`.

**Architecture:** Add `source_type: "kisa" | "vendor"` to `vulnerabilities`. Extract the current `kisa-view.tsx` fetch/match logic into a `useNoticeData` hook and its approve/reject logic into `notice-actions.ts`, then extract its list+detail-panel UI into a shared `NoticeReviewBoard` component parameterized by `sourceType`. `kisa-view.tsx` and a new `vendor-view.tsx` become thin wrappers around `NoticeReviewBoard`. `patch-view.tsx` is rewritten from an asset-centric table to a notice-centric table (rows = approved `vulnerabilities`, reusing `useNoticeData`).
...
- Keep `ViewKey` values `"kisa"` and `"patch"` unchanged (only add `"vendor"`) so existing `notifications.link_view` values and any stored state referencing them keep working.
...
- Produces: `useNoticeData(sourceType?: "kisa" | "vendor")` returning `{ vulns: Vulnerability[], setVulns, assets: Asset[], matchMap: Map<string, Asset[]>, loading: boolean, refresh: () => void }`. Also re-exports `type Vulnerability`, `type Asset`. Consumed by Tasks 4 and 8.
...
export function useNoticeData(sourceType?: Vulnerability["source_type"]) {
...
- Produces: `NoticeReviewBoard({ sourceType, title, description, icon, onNavigate })`. Consumed by Task 5 (`kisa-view.tsx`) and Task 6 (`vendor-view.tsx`).
...
export function NoticeReviewBoard({
  ...
  const { vulns, setVulns, matchMap, loading } = useNoticeData(sourceType)
...
git commit -m "feat: add shared NoticeReviewBoard component for KISA/vendor review screens"
```
...
Run: `pnpm dev`, open the app, navigate to "KISA 취약점 공지" (still under its old nav slot at this point — Task 7 moves it into the group).
Expected: same behavior as before this refactor — list + detail panel, filters, approve/reject for admin, "알림 수신 확인" for non-admin. Only rows with `source_type = 'kisa'` should appear (likely few or zero rows until Task 9 lets you register KISA-tagged rows — that's expected at this point in the plan).
...
**Interfaces:**
- Consumes: `NoticeReviewBoard` (Task 4).
- Produces: `VendorView({ onNavigate })`. Consumed by Task 7 (`app/page.tsx`).
...
Run: `pnpm build`
Expected: build succeeds (file is unused until Task 7 wires it in — that's fine, it's still type-checked).
...
### Task 7: Navigation — `nav.ts` group + `page.tsx` routing

**Files:**
- Modify: `components/portal/nav.ts:18-32` (`ViewKey`) and `components/portal/nav.ts:65-86` (`NAV_ITEMS`)
- Modify: `app/page.tsx:22` (import) and `app/page.tsx:70-73` (`renderView` switch)

**Interfaces:**
- Consumes: `VendorView` (Task 6).
- Produces: `ViewKey` now includes `"vendor"`; sidebar shows a "취약점 공지" group with 3 children.

- [ ] **Step 1: Add `"vendor"` to `ViewKey`**
...
- [ ] **Step 3: Wire `VendorView` into `app/page.tsx`**

Add the import next to the `KisaView` import (`app/page.tsx:21-22`):
...
Add a case next to the `"kisa"` case in `renderView()` (`app/page.tsx:70-73`):
...
Expected: sidebar shows a "취약점 공지" group (expandable) containing "KISA 취약점 공지" / "제조사 취약점 공지" / "승인된 취약점 공지" (still showing the old patch-view content until Task 8). Clicking "제조사 취약점 공지" renders `VendorView` with only `source_type = 'vendor'` rows (i.e. all current seed data, since it all predates the `source_type` column and defaulted to `'vendor'`).

- [ ] **Step 6: Commit**

```bash
git add components/portal/nav.ts app/page.tsx
git commit -m "feat: add vendor nav entry and group KISA/vendor/approved under 취약점 공지"
```

---

### Task 8: Rewrite `patch-view.tsx` as the notice-centric "승인된 취약점 공지" board

**Files:**
- Modify: `components/pages/patch-view.tsx` (full replace)

**Interfaces:**
- Consumes: `useNoticeData` (Task 2, called with no `sourceType` argument to get all approved notices regardless of source).
- Produces: `PatchView({ onNavigate })` — same public signature, so `app/page.tsx`'s `case "patch"` (Task 7) keeps working unchanged.

This flips the table from asset-centric rows to notice-centric rows (rows = `vulnerabilities` where `approval === "승인완료"`), keeping stats cards, search, filters, sort, column visibility, and Excel export.
...
type ColKey = "severity" | "cve" | "title" | "sourceType" | "source" | "product" | "mapped"
...
export function PatchView({ onNavigate }: { onNavigate?: (view: ViewKey) => void }) {
  ...
  const filteredSorted = useMemo(() => {
    return [...approved]
      .filter((v) => {
        ...
        return matchesQuery && matchesSeverity && matchesSourceType
      })
      .sort((a, b) => {
  ...
  }, [approved, query, severity, sourceType, sortKey, sortDir, matchMap])
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        icon={ShieldCheck}
        title="승인된 취약점 공지"
        description="KISA·제조사에서 승인 완료된 취약점 공지를 전사 자산 매핑 기준으로 조회합니다. 신규 미승인 공지는 KISA/제조사 취약점 공지 화면에서 검토·승인하세요."
        action={
          onNavigate ? (
      ...
      <SectionCard
        title="승인된 공지 목록"
        subtitle="승인 완료된 취약점 공지와 매핑된 자산 현황입니다"
        icon={ShieldCheck}
        action={
          <div className="flex items-center gap-1.5">
            <ExportExcelButton
      ...
      >
        ...
        <p className="mb-3 text-sm text-muted-foreground">
          총 <span className="font-mono font-semibold text-foreground">{pagination.pageItems.length}</span>건
...
Run: `pnpm build`
Expected: build succeeds. Watch for unused-import errors (`X` from lucide-react is not used in this file, unlike the old version — make sure it's not imported) and colspan typing on raw `<td>` (should be fine, it's a plain DOM element, not the `Td` wrapper).

- [ ] **Step 3: Manual check**

Run: `pnpm dev`, navigate to "승인된 취약점 공지" via the new nav group (Task 7 already wired this).
Expected:
- Rows are now vulnerabilities (approved only), not assets.
- Stats cards show 전체/CRITICAL/HIGH/미매핑 counts derived from approved notices.
- Search box filters by title/CVE/product/source.
- 상세 필터 아코디언 lets you filter by 심각도 and 출처 유형 (전체/KISA/제조사).
...
- Excel export downloads a file with the 7 listed columns.
- "KISA 취약점 공지 바로가기" / "제조사 취약점 공지 바로가기" buttons navigate correctly.

- [ ] **Step 4: Commit**

```bash
git add components/pages/patch-view.tsx
git commit -m "refactor: patch-view.tsx becomes notice-centric 승인된 취약점 공지 board"
```

---

### Task 9: Manual registration — `source_type` + collector defaults
...
**Interfaces:**
- No new exports; this task only changes internal state/insert payloads.
...
```tsx
          <p className="mt-3 text-[11px] text-muted-foreground">
            등록된 공지는 &apos;승인대기&apos; 상태로 출처 유형에 맞는 화면(KISA 취약점 공지 / 제조사 취약점 공지)에 나타납니다. 그곳에서 검토 후 승인하면 승인된 취약점 공지에 반영됩니다.
          </p>
```
...
and the `notices.push({...})` inside `collectTmaxSoft` (around line 151-161):
...
- Register one row with 출처 유형 = KISA, one with 제조사.
Expected: the KISA one appears only in "KISA 취약점 공지", the 제조사 one only in "제조사 취약점 공지" (both start as `승인대기`).
Then run "즉시 수집" for Apache Tomcat/JEUS/WebtoB (or check existing collected rows in Supabase) and confirm those rows have `source_type = 'vendor'`.
...
1. Register a manual vulnerability with 출처 유형 = KISA on an existing seeded product name (e.g. "Apache Tomcat").
...
4. Go to "승인된 취약점 공지" — confirm the approved KISA row appears, with 출처 유형 badge = KISA, and expanding "매핑 자산" shows the matched asset.
5. Register a second manual vulnerability with 출처 유형 = 제조사, go to "제조사 취약점 공지", reject it — confirm it disappears from both "제조사 취약점 공지" (now shows 반려) and does not appear in "승인된 취약점 공지".
6. Switch role to 일반 사용자 (`RoleToggle`) — confirm KISA/제조사 approve/reject buttons are hidden and replaced by "알림 수신 확인", and "승인된 취약점 공지" is still visible (no admin-only restriction).
